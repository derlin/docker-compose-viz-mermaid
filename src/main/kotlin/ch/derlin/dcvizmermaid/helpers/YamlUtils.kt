...
object YamlUtils {
    ...
    inline fun <reified T> YAML.getListByPath(
        path: String,
        default: List<T>,
    ): List<T> =
        getByPath(path)?.let { value ->
            require(value is List<*> && value.all { it is T }) { "Unexpected type for list $value" }
            value as List<T>
        } ?: default

    fun YAML.getListOrMapByPath(path: String): ListOrMap? =
        when (val value = this.getByPath(path)) {
            is List<*> -> OrList(value as List<String>)
            is Map<*, *> -> OrMap(value as YAML)
            else -> null
        }

    fun <T : Any> YAML.getByPath(
        path: String,
        type: KClass<T>,
    ): T? =
        getByPath(path)?.let {
